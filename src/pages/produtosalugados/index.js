    ...
    return (
        <Flex flexDirection="column" h="100%" backgroundColor="#322F38" overflowY="hidden" overflow="auto"> 
            ...
            <Flex flexDirection="column" padding="10">
                {data.map(produto => produto.datasAlugadas.map((data, index) => <Card key={`${produto.id}_${index}`} imagem={produto.imagem} idp={produto.id} avaliacao={produto.avaliacao} locatario={produto.locatario} avaliacao={produto.avaliacao} datas={data} local={produto.local} preco={produto.preco}/>))}
            </Flex>
        </Flex>
    )
}